...
# MAGIC %pip install -U --quiet databricks-sdk==0.36.0 databricks-agents mlflow-skinny==2.18.0 mlflow==2.18.0 mlflow[gateway]==2.18.0 databricks-vectorsearch langchain==0.2.1 langchain_core==0.2.5 langchain_community==0.2.4 opencv-python==4.10.0.84
# MAGIC dbutils.library.restartPython()

# COMMAND ----------

# MAGIC %md 
# MAGIC ### Create source dataset by extracting Keskkonnaagentuur's website

# COMMAND ----------

# MAGIC %run ./_resources/00-init-taastuvenergia $recreate_source_table=false

# COMMAND ----------

# MAGIC %sql 
# MAGIC -- The dataset for your knowledge base has been loaded for you in the init notebook.
# MAGIC SELECT id, url, content FROM databricks_scraped_taastuvenergia_with_images; 
...
# source_table_fullname = f"{catalog}.{db}.databricks_pdf_taastuvenergia"
source_table_fullname = f"{catalog}.{db}.databricks_scraped_taastuvenergia_with_images"
# Where we want to store our index
# vs_index_fullname = f"{catalog}.{db}.databricks_pdf_taastuvenergia_vs_index"
vs_index_fullname = f"{catalog}.{db}.databricks_scraped_taastuvenergia_with_images_vs_index"
...
chain_config = {
    ...
    "vector_search_index": vs_index_fullname, #f"{catalog}.{db}.databricks_pdf_riigieelarve_vs_index",
    "llm_prompt_template": """You are an assistant that answers questions about renewable energiy. Use the following pieces of retrieved context to answer the question in Estonian. Some pieces of context may be irrelevant, in which case you should not use them to form the answer. If you do not know the answer to a question, you truthfully say you do not know.\n\nContext: {context}""",
}

input_example = {"messages": [ {"role": "user", "content": "Mis on bioenergia?"}]}
...
instructions_to_reviewer = f"""## Instructions for Testing the Databricks Taastuvenergia Assistant chatbot
...
# not allowed in solita sandbox: Invalid Inputs: Passed in parameters are not found. ACLs for inference-endpoint are disabled or not available in this tier
# user_list = ["anne.ott@solita.fi"]
