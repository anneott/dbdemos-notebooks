...
volume_folder =  f"/Volumes/{catalog}/{db}/volume_taastuvenergia"
...
upload_local_pdf_to_volume(source_file_path="../data/keskkonnaportaal_taastuvenergia.pdf" , volume_path=volume_folder+"/databricks-pdf") # new code!
...
# MAGIC CREATE TABLE IF NOT EXISTS databricks_pdf_taastuvenergia (
...
(spark.readStream.table('pdf_raw')
  ...
  .writeStream
    ...
    .table('databricks_pdf_taastuvenergia').awaitTermination())
...
if spark.catalog.tableExists(f'{catalog}.{db}.databricks_taastuvenergia'):
  (spark.readStream.option("skipChangeCommits", "true").table('databricks_taastuvenergia') #skip changes for more stable demo
      .withColumn('embedding', get_embedding("content"))
...
# MAGIC %sql
# MAGIC SELECT * FROM databricks_pdf_taastuvenergia WHERE url like '%.pdf' limit 10
...
vs_index_fullname = f"{catalog}.{db}.databricks_pdf_taastuvenergia_self_managed_vs_index"
