...
# MAGIC CREATE TABLE IF NOT EXISTS databricks_pdf_riigieelarve (
...
(spark.readStream.table('pdf_raw')
  ...
  .writeStream
    ...
    .table('databricks_pdf_riigieelarve').awaitTermination())


# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM main__build.dbdemos_rag_chatbot.databricks_pdf_riigieelarve WHERE url like '%.pdf' limit 10

# COMMAND ----------


#Let's also add our documentation web page from the simple demo (make sure you run the quickstart demo first)
if spark.catalog.tableExists(f'{catalog}.{db}.databricks_riigieelarve'):
  (spark.readStream.option("skipChangeCommits", "true").table('databricks_riigieelarve') #skip changes for more stable demo
      .withColumn('embedding', get_embedding("content"))
...
# MAGIC %sql
# MAGIC SELECT * FROM databricks_pdf_riigieelarve WHERE url like '%.pdf' limit 10
