...
# MAGIC %run ./config_ooliblikas_simple

# COMMAND ----------

# MAGIC %run ../_resources/00-init-ooliblikas $recreate_source_table=false

# COMMAND ----------

# MAGIC %sql 
# MAGIC -- The dataset for your knowledge base has been loaded for you in the init notebook.
# MAGIC SELECT * FROM databricks_pdf_ooliblikas
...
source_table_fullname = f"{catalog}.{db}.databricks_pdf_ooliblikas"
# Where we want to store our index
vs_index_fullname = f"{catalog}.{db}.databricks_pdf_ooliblikas_vs_index"
...
# MAGIC %md
# MAGIC Neljas vastus on õige teema kohta:
# MAGIC   
# MAGIC   Ööliblikate seire toimus 2020. aastal kuues  paigas. Kuuest vaatlusalast k aks on olnud \nkasutusel juba alates aasta st 2003: Nigula Looduskaitseala keskus Pärnumaal  ning Puka \nValgamaal. Kolmandal vaatlusalal, Salinõmme külas Hiiumaal, alustati seirega aastal 2005. Neljas  vaatlusala, Sääre küla Sõrve poolsaare lõunatipus Saaremaal, lisati seireprogrammi \naastal 2012 ; viies, Piilse küla Ida -Virumaal aastal 2014  ning kuues, Reopalu küla Järvamaal \naastal 2020 .
# MAGIC
# MAGIC
# MAGIC
# MAGIC   Kuidas similarity score arvutatakse?
# MAGIC   https://docs.databricks.com/aws/en/generative-ai/vector-search

# COMMAND ----------

question = "Mis on ööliblikate seirealad Eestis aastal 2020?"
# question = "Kus seiret ei teostatud?"
# question = "Kirjelda ööbliblikate seisu 2020.aastal?"
# question = "Mis on hinnang ööliblikate arvukusele praegu? Kas ööliblikaid jääb ajas vähemaks või tuleb juurde?"
...
chain_config = {
    ...
    "vector_search_index": f"{catalog}.{db}.databricks_pdf_ooliblikas_vs_index",
    "llm_prompt_template": """You are an assistant that answers questions in Estonian language. Use the following pieces of retrieved context to answer the question. Some pieces of context may be irrelevant, in which case you should not use them to form the answer.\n\nContext: {context}""",
}
...
# MAGIC %md
# MAGIC Järgnev koodijupp leiab top 3 similarity serachi järgi, aga õige vastus peitub neljandas:
# MAGIC
# MAGIC
# MAGIC
# MAGIC   
...
relevant_docs = (vector_search_as_retriever | RunnableLambda(format_context)| StrOutputParser()).invoke('Mis on ööliblikate seirealad Eestis aastal 2020?')
...
answer = (prompt | model | StrOutputParser()).invoke({'question':'Mis on ööliblikate seirealad Eestis aastal 2020?', 'context': ''})
display(answer)
...
# Let's give it a try:
input_example = {"messages": [ {"role": "user", "content": "Mis on ööliblikate seirealad Eestis aastal 2020?"}]}
