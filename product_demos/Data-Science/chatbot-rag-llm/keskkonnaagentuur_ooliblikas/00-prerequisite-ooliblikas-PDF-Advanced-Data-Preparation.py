...
# MAGIC %run ./_resources/00-init-ooliblikas # $reset_all_data=false
...
# MAGIC %sql
# MAGIC CREATE VOLUME IF NOT EXISTS volume_ooliblikas;

# COMMAND ----------

# DBTITLE 1,Our pdf or docx files are available in our Volume (or DBFS)
# List our raw PDF docs
volume_folder =  f"/Volumes/{catalog}/{db}/volume_ooliblikas"
...
upload_local_pdf_to_volume(source_file_path="data/aruanne-2020-ooliblikad.pdf" , volume_path=volume_folder+"/databricks-pdf") 
upload_local_pdf_to_volume(source_file_path="data/ööliblikad_koondhinnang2020.pdf" , volume_path=volume_folder+"/databricks-pdf") 
upload_local_pdf_to_volume(source_file_path="data/Vormikohane aruanne.pdf" , volume_path=volume_folder+"/databricks-pdf") 
...
# MAGIC %sql 
# MAGIC TRUNCATE TABLE main__build.dbdemos_rag_chatbot.pdf_raw;
...
# MAGIC %sql 
# MAGIC SELECT * FROM main__build.dbdemos_rag_chatbot.pdf_raw LIMIT 10
...
# MAGIC %sql
# MAGIC -- drop table databricks_pdf_riigieelarve;
# MAGIC --Note that we need to enable Change Data Feed on the table to create the index
# MAGIC CREATE TABLE IF NOT EXISTS databricks_pdf_ooliblikas (
...
(spark.readStream.table('pdf_raw')
  ...
  .writeStream
    ...
    .table('databricks_pdf_ooliblikas').awaitTermination())


# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM main__build.dbdemos_rag_chatbot.databricks_pdf_taastuvenergia WHERE url like '%.pdf' limit 10

# COMMAND ----------


#Let's also add our documentation web page from the simple demo (make sure you run the quickstart demo first)
if spark.catalog.tableExists(f'{catalog}.{db}.databricks_ooliblikas'):
  (spark.readStream.option("skipChangeCommits", "true").table('databricks_ooliblikas') #skip changes for more stable demo
      .withColumn('embedding', get_embedding("content"))
...
# MAGIC %sql
# MAGIC SELECT * FROM databricks_pdf_ooliblikas WHERE url like '%.pdf' limit 10

# COMMAND ----------

# MAGIC %md
# MAGIC # For basic App Stop here and move to notebook 00-ooliblikas-Mosaic-AI-Agent-Demo
...
vs_index_fullname = f"{catalog}.{db}.databricks_pdf_ooliblikas_self_managed_vs_index"
...
question = "Kus teostati ööliblikate seiret 2020. aastal?"
